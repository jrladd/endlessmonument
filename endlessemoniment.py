...
filename=open("/home/jrladd/twitterbots/epithalamibot/epithalamion.txt", 'r')
...
shortlines_file=open('/home/jrladd/twitterbots/epithalamibot/shortlines.txt', 'r')
shortlines=shortlines_file.readlines()
shortlines_file.close()

longlines_file=open('/home/jrladd/twitterbots/epithalamibot/longlines.txt', 'r')
...
def reply_to_tweets():
	our_tweets=api.user_timeline(count=5)
	ids=[]
	for tweet in our_tweets:
		ids.append(tweet.id)
	m=api.mentions_timeline(max(ids))
	for s in m:
		status=s.text
		tweet_id=s.id_str
		...
		try:
			...
			api.update_status(new_reply, tweet_id)
...
start_in_march=range(60,366) + range(1,60)
